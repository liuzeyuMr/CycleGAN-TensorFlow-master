...
class Reader():
  def __init__(self, tfrecords_file, image_size=256,
    ...
    self.min_queue_examples = min_queue_examples#1000
    self.batch_size = batch_size#1
    self.num_threads = num_threads#8
    self.reader = tf.TFRecordReader() #创建一个reader来读取TFRecord文件中的样例
    self.name = name
  ...
  def feed(self):
    ...
    with tf.name_scope(self.name):
      filename_queue = tf.train.string_input_producer([self.tfrecords_file]) #根据文件名生成一个队列
      # reader = tf.TFRecordReader()

      _, serialized_example = self.reader.read(filename_queue) #从文件中读出一个样例 返回文件名和文件
      #解析读入的样例
      ...
      image = tf.image.decode_jpeg(image_buffer, channels=3)#将字符串解析成jpg图片
      image = self._preprocess(image) #进行预处理
      # print(image.shape)
      #将batch_size张图片保存到images
