...
def c7s1_k(input, k, reuse=False, norm='instance', activation='relu', is_training=True, name='c7s1_k'):
  ...
  Args:
    input: 4D tensor
    k: integer, number of filters (output depth) #64
  ...
  with tf.variable_scope(name, reuse=reuse):
    weights = _weights("weights",
      shape=[7, 7, input.get_shape()[3], k]) #得到fiture个数 [7,7,3,64]

    padded = tf.pad(input, [[0,0],[3,3],[3,3],[0,0]], 'REFLECT')
    conv = tf.nn.conv2d(padded, weights,
        strides=[1, 1, 1, 1], padding='VALID')

    normalized = _norm(conv, is_training, norm)  #归一化

    if activation == 'relu':
      output = tf.nn.relu(normalized) #激活
    if activation == 'tanh':
  ...
  with tf.variable_scope(name, reuse=reuse):
    with tf.variable_scope('layer1', reuse=reuse):
      weights1 = _weights("weights1",
        shape=[3, 3, input.get_shape()[3], k]) #[3,3,256,256]
...
def n_res_blocks(input, reuse, norm='instance', is_training=True, n=9):
  depth = input.get_shape()[3] #256
...
def uk(input, k, reuse=False, norm='instance', is_training=True, name=None, output_size=None):
  ...
  with tf.variable_scope(name, reuse=reuse):
    ...
    if not output_size:
      output_size = input_shape[1]*2 #输出图片比输入图片大一倍
...
def Ck(input, k, slope=0.2, stride=2, reuse=False, norm='instance', is_training=True, name=None):
  ...
  with tf.variable_scope(name, reuse=reuse):
    weights = _weights("weights",
      shape=[4, 4, input.get_shape()[3], k]) #[4,4,3,64]
...
def _instance_norm(input):
  """ Instance Normalization 这个也就是进行了归一化，但是没有参数去学习，不同于标准的batchnormal
  """
